...
pub fn get_implementation(name: String) -> Option<FuncImpl> {
    match name.as_str() {
        ...
        "argv" => Some(Box::new(|interp, db, info| {
            let i = interp.get_local("argv", "it")?;
            match i {
                I32(ind, _) => Ok(Str(
...
fn lazy_operator(binding: i32, assoc: Direction, lazy_left: bool, lazy_right: bool) -> Semantic {
    Semantic::Operator {
        binding,
        assoc,
        laziness: Some((lazy_left, lazy_right)),
    }
}
...
pub fn get_externs(_db: &dyn Compiler) -> Result<HashMap<String, Extern>, TError> {
    ...
    let mut externs = vec![
        ...
        Extern {
            name: ";".to_string(),
            semantic: lazy_operator(20, Right, true, true),
            ty: Function {
